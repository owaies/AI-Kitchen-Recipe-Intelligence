  ...
  return (
    <div className="cook-mode" role="dialog" aria-modal="true" aria-label="Cook mode">
      ...
      <div className="cook-panel">
        ...
        <AnimatePresence mode="wait" initial={false}><motion.div key={step} className="cook-step" initial={{opacity:0,x:24,filter:"blur(5px)"}} animate={{opacity:1,x:0,filter:"blur(0)"}} exit={{opacity:0,x:-24,filter:"blur(5px)"}} drag="x" dragConstraints={{left:0,right:0}} dragElastic={.28} onDragEnd={(_,info) => { if(info.offset.x < -55 && step < total - 1) setStep(value => value + 1); if(info.offset.x > 55 && step > 0) setStep(value => value - 1); }} transition={{duration:.32,ease:[.22,.8,.22,1]}}><span className="cook-step-number">{String(step + 1).padStart(2, "0")}</span><div><span className="eyebrow">Method</span><h2>{recipe.steps[step] ?? "Your recipe is ready."}</h2><p>Take your time and check the texture before moving to the next step.</p></div></motion.div></AnimatePresence>
