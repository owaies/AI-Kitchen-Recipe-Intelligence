...
create index if not exists pantry_items_user_expiry_idx on pantry_items(user_id, expires_on);
create index if not exists meal_plans_user_date_idx on meal_plans(user_id, plan_date);
create index if not exists shopping_items_user_purchased_idx on shopping_items(user_id, is_purchased);
